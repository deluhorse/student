...
class excel_util(object):
    ...
    @staticmethod
    def parse_excel(file_data, start_line=1):
        """
        解析excel
        :param file_data:
        :param start_line
        ...
        result = []
        try:
            workbook = xlrd.open_workbook(file_contents=file_data[0]['body'])
        except Exception as e:
            logger.exception(e)
            return result

        table = workbook.sheets()[0]
        raw_count = table.nrows

        for i in range(start_line, raw_count):
